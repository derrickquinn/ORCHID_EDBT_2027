...
void GpuIndexIVF::searchImpl_(
    ...
    DeviceTensor<uint8_t, 1, true> selectorBitmapTensor;
    const uint8_t* selectorBitmapPtr = nullptr;
    size_t selectorBitmapStride = 0;
    ...
    if (selectorBitmapPtr) {
        auto ivfFlatPtr = std::dynamic_pointer_cast<IVFFlat>(baseIndex_);
        FAISS_THROW_IF_NOT_MSG(
                ivfFlatPtr,
                "GpuIndexIVF: per-query bitmap selectors are supported only for IVFFlat");

        ivfFlatPtr->searchWithBitmaps(
                quantizer,
                queries,
                use_nprobe,
                k,
...
void GpuIndexIVF::search_preassigned(
    ...
    if (params && params->sel2d) {
        ...
        auto bitmapSelector =
                dynamic_cast<const IDSelector2DBitmap*>(params->sel2d);
        FAISS_THROW_IF_NOT_MSG(
                bitmapSelector,
                "GpuIndexIVF currently supports only IDSelector2DBitmap selectors");
        FAISS_THROW_IF_NOT_MSG(
    ...
    if (selectorBitmapPtr) {
        ...
        ivfFlatPtr->searchPreassignedWithBitmaps(
                quantizer,
                vecsDevice,
                distanceDevice,
                assignDevice,
