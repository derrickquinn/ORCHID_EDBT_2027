...
struct IVFFlatScanFiltered {
    static __device__ void scan(
            ...
            int dim,
            float* distanceOut,
            const idx_t* listIndices,
            const uint8_t* queryBitmap) {
        int limit = utils::divDown(dim, Codec::kDimPerIter);
        auto warpId = threadIdx.x / kWarpSize;
        ...
        idx_t vecsPerWarp = utils::divUp(numVecs, kIVFFlatScanWarps);
        idx_t vecStart = vecsPerWarp * warpId;
        idx_t vecEnd = min(vecsPerWarp * (idx_t)(warpId + 1), numVecs);

        for (idx_t vec = vecStart; vec < vecEnd; ++vec) {
            int allowedInt = 1;
            if (laneId == 0) {
                idx_t vectorId = listIndices[vec];
                auto byteIndex = static_cast<size_t>(
                        (static_cast<uint64_t>(vectorId)) >> 3);
                uint8_t mask = uint8_t(1) << (vectorId & 7);
                allowedInt = (queryBitmap[byteIndex] & mask) != 0;
            }
            allowedInt = __shfl_sync(0xffffffff, allowedInt, 0);

            if (!allowedInt) {
                if (laneId == 0) {
                    distanceOut[vec] = Metric::kMaxDistance;
                }
                continue;
            }
...
template <typename Codec, typename Metric>
__global__ void ivfFlatScanFiltered(
        Tensor<float, 2, true> queries,
        bool useResidual,
        Tensor<float, 3, true> residualBase,
        Tensor<idx_t, 2, true> listIds,
        void** allListData,
        const idx_t* const* allListIndices,
        idx_t* listLengths,
        Codec codec,
        Metric metric,
        Tensor<idx_t, 2, true> prefixSumOffsets,
        Tensor<float, 1, true> distance,
        const uint8_t* selectorBitmaps,
        size_t bitmapStride) {
    extern __shared__ float smem[];
    ...
    auto residualBaseSlice = residualBase[queryId][probeId].data();
    const idx_t* listIndices =
            allListIndices ? allListIndices[listId] : nullptr;
    const uint8_t* queryBitmap =
            selectorBitmaps + (size_t)queryId * bitmapStride;

    codec.initKernel(smem, dim);
    __syncthreads();

    IVFFlatScanFiltered<Codec, Metric>::scan(
            query,
            useResidual,
            residualBaseSlice,
            vecs,
...
void runIVFFlatScanTile(
    ...
    bool useSelector = selectorBitmaps != nullptr;
    const idx_t* const* listIndicesPtr = useSelector
            ? reinterpret_cast<const idx_t* const*>(listIndices.data())
            : nullptr;
...
#define RUN_IVF_FLAT_FILTERED                                      \
    do {                                                           \
        ivfFlatScanFiltered<<<                                     \
                grid,                                              \
                block,                                             \
                codec.getSmemSize(dim),                            \
                stream>>>(                                         \
                queries,                                           \
                useResidual,                                       \
                residualBase,                                      \
...
void runIVFFlatScan(
        ...
        Tensor<idx_t, 2, true>& outIndices,
        GpuResources* res,
        const uint8_t* queryBitmaps,
        size_t bitmapBytesPerQuery) {
    auto stream = res->getDefaultStreamCurrentDevice();

    if (queryBitmaps) {
        FAISS_THROW_IF_NOT_MSG(
                bitmapBytesPerQuery > 0,
                "Bitmap stride must be > 0 when query bitmaps are provided");
        FAISS_THROW_IF_NOT_MSG(
                indicesOptions == INDICES_64_BIT,
                "Per-query filtering currently requires 64-bit indices on GPU");
    }
    ...
    for (idx_t query = 0; query < queries.getSize(0); query += queryTileSize) {
        ...
        const uint8_t* tileBitmaps = queryBitmaps
                ? queryBitmaps + (size_t)query * bitmapBytesPerQuery
                : nullptr;
