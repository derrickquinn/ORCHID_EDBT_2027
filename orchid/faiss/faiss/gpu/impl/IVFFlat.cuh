...
class IVFFlat : public IVFBase {
   public:
    ...
    /// Same as search() but applies per-query bitmap filters.
    void searchWithBitmaps(
            Index* coarseQuantizer,
            Tensor<float, 2, true>& queries,
            int nprobe,
            int k,
    ...
    /// Same as searchPreassigned but applies per-query bitmap filters.
    void searchPreassignedWithBitmaps(
            Index* coarseQuantizer,
            Tensor<float, 2, true>& vecs,
            Tensor<float, 2, true>& ivfDistances,
            Tensor<idx_t, 2, true>& ivfAssignments,
