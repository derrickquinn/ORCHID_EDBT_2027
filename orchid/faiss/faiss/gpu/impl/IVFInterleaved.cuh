...
__global__ void ivfInterleavedScan(
    ...
    if constexpr ((NumWarpQ == 1 && NumThreadQ == 1) || NumWarpQ >= kWarpSize) {
        ...
        for (idx_t queryId = blockIdx.y; queryId < queries.getSize(0);
            ...
            const uint8_t* queryBitmap = selectorBitmaps
            ...
            const idx_t* listIndicesBase = allListIndices[listId];
            ...
            for (idx_t block = warpId; block < numBlocks; block += kNumWarps) {
                ...
                if (valid && queryBitmap) {
                    idx_t vecId = listIndicesBase[vec];
                    uint8_t mask = uint8_t(1) << (vecId & 7);
                    valid = (queryBitmap[vecId >> 3] & mask) != 0;
                }
...
void runIVFInterleavedScan(
        ...
        GpuResources* res,
        const uint8_t* queryBitmaps = nullptr,
        size_t bitmapBytesPerQuery = 0);
