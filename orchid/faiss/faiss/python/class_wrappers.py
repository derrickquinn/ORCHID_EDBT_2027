...
def replace_method(the_class, name, replacement, ignore_missing=False):
    ...
    if orig_method.__name__ == 'replacement_' + name:
        ...
        return
    existing_c = getattr(the_class, name + '_c', None)
    if existing_c is None or getattr(existing_c, '__name__', '').startswith('replacement_'):
...
def handle_IDSelector2DBitmap(the_class):
    the_class.original_init = the_class.__init__

    def replacement_init(self, *args, **kwargs):
        if len(args) in (1, 2) and args[0].__class__ != the_class:
            bitmap = np.ascontiguousarray(args[0], dtype='uint8')
            if bitmap.ndim != 2:
                raise ValueError("bitmap must be a 2D array of dtype uint8")
            nq, bytes_per_bitmap = bitmap.shape
            if len(args) == 2:
                universe = args[1]
            else:
                universe = kwargs.pop('N', None)
            if kwargs:
                raise TypeError("unexpected keyword arguments: %s" %
                                ", ".join(kwargs.keys()))
            if universe is None:
                universe = bytes_per_bitmap * 8
            self.original_init()
            self.base = faiss.swig_ptr(bitmap)
            self.bytes_per_bitmap = bytes_per_bitmap
            self.N = universe
            self.nq = nq
            add_to_referenced_objects(self, bitmap)
            return
        if kwargs:
            raise TypeError("unexpected keyword arguments: %s" %
                            ", ".join(kwargs.keys()))
