...
/***********************************************************
 * Callbacks for IDSelector2D
 ***********************************************************/

PyCallbackIDSelector2D::PyCallbackIDSelector2D(PyObject* callback)
        : callback(callback) {
    PyThreadLock gil;
    Py_INCREF(callback);
}

bool PyCallbackIDSelector2D::is_member(faiss::idx_t q, faiss::idx_t id) const {
    FAISS_THROW_IF_NOT((id >> 32) == 0);
    PyThreadLock gil;
    PyObject* result = PyObject_CallFunction(
            callback, "(LL)", (long long)q, (long long)id);
    if (result == nullptr) {
        FAISS_THROW_MSG("propagate py error");
    }
    bool b = PyObject_IsTrue(result);
...
PyCallbackIDSelector2D::~PyCallbackIDSelector2D() {
    PyThreadLock gil;
    Py_DECREF(callback);
}
