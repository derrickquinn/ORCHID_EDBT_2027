...
#include <faiss/impl/IDSelector2D.h>  //added 
...
struct IVFFlatScanner : InvertedListScanner {
    ...
    // NEW: setters (called from IndexIVF::search_preassigned)
    void set_query_idx(idx_t q) override { qidx = q; }
    void set_selector2d(const IDSelector2D* s) override { sel2d = s; }
    ...
    void scan_codes_range(
        ...
        for (size_t j = 0; j < list_size; j++) {
            const float* yj = list_vecs + vd.d * j;
             ...
             // --- NEW allow check ---
            // if (sel2d) {
            //     if (!sel2d->is_member(qidx, ids[j])) { continue; }
            // } else if (use_sel) {
            //     if (!sel->is_member(ids[j])) { continue; }
            // }
            // Check regular selector first (includes 2D adapters)
            if (use_sel && sel) {
                if (!sel->is_member(ids[j])) { continue; }
            } else if (sel2d) {
                // Fallback direct 2D selector (shouldn't be used in current flow)
                if (!sel2d->is_member(qidx, ids[j])) { continue; }
            }
            // --- END NEW ---
...
InvertedListScanner* IndexIVFFlat::get_InvertedListScanner(
        bool store_pairs,
        const IDSelector* sel,
        const IVFSearchParameters* params) const {  // Use params!
    Run_get_InvertedListScanner run;
    auto* scanner = dispatch_VectorDistance(
            d, metric_type, metric_arg, run, this, store_pairs, sel);
    
    // Set 2D selector if available
    if (params && params->sel2d) {
        scanner->set_selector2d(params->sel2d);
    }
