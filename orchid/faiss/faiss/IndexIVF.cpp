// modified indexivffile that supports 2d selector
...
#include <faiss/impl/IDSelector2D.h>   // 2D selector support

namespace faiss {

static thread_local idx_t tls_query_base = 0;
...
void IndexIVF::search_preassigned(
    ...
    [[maybe_unused]] bool do_parallel =
            ...
            (pmode == 0 ? false : pmode == 3 ? n > 1 : pmode == 1 ? nprobe > 1
                                                                  : nprobe * n > 1);

    void* inverted_list_context =
            params ? params->inverted_list_context : nullptr;

    const IDSelector2D* sel2d = params ? params->sel2d : nullptr;
    // printf("DEBUG: sel2d = %p, taking %s path\n", sel2d, sel2d ? "2D selector" : "legacy");
...
#pragma omp parallel if (do_parallel) reduction(+ : nlistv, ndis, nheap)
    {
        ...
        /*****************************************************
         * Branch 1: legacy path (single 1D selector)
         ******************************************************/
        if (!sel2d) {
            // One per-thread baseline scanner object is fine to keep around,
            // but we will build a per-query scanner_q carrying the correct selector.
            std::unique_ptr<InvertedListScanner> scanner_base(
                    get_InvertedListScanner(store_pairs, sel, params));

            auto scan_one_list = [&](InvertedListScanner* scanner_local,
                                     idx_t key,
                                     float coarse_dis_i,
                                     float* simi,
                                     idx_t* idxi,
                ...
                scanner_local->set_list(key, coarse_dis_i);
                nlistv++;
                ...
                try {
                    if (invlists->use_iterator) {
                        ...
                        nheap += scanner_local->iterate_codes(
                                it.get(), simi, idxi, k, list_size);
                        return list_size;
                    } else {
                        size_t list_size = invlists->list_size(key);
                        if (list_size > list_size_max) list_size = list_size_max;
                        ...
                        if (selr) {
                            ...
                            list_size = jmax - jmin;
                            if (list_size == 0) return (size_t)0;
                            codes += jmin * code_size;
                            ids += jmin;
                        }

                        nheap += scanner_local->scan_codes(
...
#pragma omp for
                for (idx_t i = 0; i < n; i++) {
                    ...
                    // Build per-query scanner with the *1D* selector (sel)
                    std::unique_ptr<InvertedListScanner> scanner_q(
                            get_InvertedListScanner(store_pairs, sel, params));

                    scanner_q->set_query_idx(i);
                    scanner_q->set_query(x + i * d);
                    ...
                    idx_t nscan = 0;
                    for (size_t ik = 0; ik < (size_t)nprobe; ik++) {
                        nscan += scan_one_list(
                                scanner_q.get(),
                                keys[i * nprobe + ik],
                                ...
                                idxi,
                                max_codes - nscan);
                        if (nscan >= max_codes) break;
                    }

                    ndis += nscan;
                    reorder_result(simi, idxi);

                    if (InterruptCallback::is_interrupted()) interrupt = true;
                }
            } else if (pmode == 1) {
...
#pragma omp for schedule(dynamic)
                for (int64_t ij = 0; ij < n * (int64_t)nprobe; ij++) {
                    size_t i = ij / nprobe;
...
#pragma omp critical
                    {
                        add_local_results(local_dis.data(),
                                          local_idx.data(),
                                          distances + i * k,
                                          labels + i * k);
                    }
...
#pragma omp single
        ...
        /*****************************************************
         * Branch 2: 2D per-query selector path
         ******************************************************/
        } else {
            auto scan_one_list_q = [&](InvertedListScanner* scanner_q,
                                       idx_t key,
                                       float coarse_dis_i,
                                       float* simi,
                                       idx_t* idxi,
                ...
                scanner_q->set_list(key, coarse_dis_i);
                nlistv++;
...
#pragma omp for
                for (idx_t i = 0; i < n; i++) {
                    ...
                    // Compose the query's selector: 2D adapter (and optional 1D sel ANDed)
                    // IDSelectorFrom2DForQ sel_q(sel2d, i);
                    IDSelectorFrom2DForQ sel_q(sel2d, tls_query_base + i);
                    const IDSelector* effective_sel = &sel_q;
                    IDSelectorAnd sel_and(nullptr, nullptr);
                    if (sel) { sel_and = IDSelectorAnd(&sel_q, sel); effective_sel = &sel_and; }

                    std::unique_ptr<InvertedListScanner> scanner_q(
                        get_InvertedListScanner(store_pairs, effective_sel, params));
...
#pragma omp for schedule(dynamic)
                for (int64_t ij = 0; ij < n * (int64_t)nprobe; ij++) {
                    ...
                    idx_t key = keys[ij];
                    if (key >= 0 && !invlists->is_empty(key, inverted_list_context)) {
                        scanner_q->set_list(key, coarse_dis[ij]);
                        if (invlists->use_iterator) {
                            std::unique_ptr<InvertedListsIterator> it(
                                    invlists->get_iterator(key, inverted_list_context));
                            size_t list_size = 0;
                            nheap += scanner_q->iterate_codes(
                                    it.get(), local_dis.data(), local_idx.data(), k, list_size);
                            ndis += list_size;
                        } else {
                            size_t list_size = invlists->list_size(key);
                            InvertedLists::ScopedCodes scodes(invlists, key);
...
void IndexIVF::reconstruct_n(idx_t i0, idx_t ni, float* recons) const {
    ...
    for (idx_t list_no = 0; list_no < nlist; list_no++) {
        ...
        for (idx_t offset = 0; offset < (idx_t)list_size; offset++) {
...
void InvertedListScanner::scan_codes_range(
    ...
    for (size_t j = 0; j < list_size; j++) {
        ...
        float dis = distance_to_code(codes);
        bool keep = !keep_max ? dis < radius : dis > radius;
        if (keep) {
            int64_t id = store_pairs ? lo_build(list_no, j) : ids[j];
            res.add(dis, id);
        }
