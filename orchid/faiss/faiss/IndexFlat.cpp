...
void IndexFlat::search(
    ...
    // Legacy global selector (may be null)
    IDSelector* sel1d = params ? params->sel : nullptr;

    // Optional per-query selector
    const IDSelector2D* sel2d = nullptr;
    if (auto sp2 = dynamic_cast<const SearchParametersSelector2D*>(params)) {
        sel2d = sp2->sel2d;
    }

    // -------- Fast path (no 2-D selector): keep existing code ----------
    if (!sel2d) {
        if (metric_type == METRIC_INNER_PRODUCT) {
            float_minheap_array_t res = {size_t(n), size_t(k), labels, distances};
            knn_inner_product(x, get_xb(), d, n, ntotal, &res, sel1d);
        } else if (metric_type == METRIC_L2) {
            float_maxheap_array_t res = {size_t(n), size_t(k), labels, distances};
            knn_L2sqr(x, get_xb(), d, n, ntotal, &res, nullptr, sel1d);
        } else {
            // unchanged behavior
            FAISS_THROW_IF_NOT(!sel1d);
            knn_extra_metrics(
                    x, get_xb(), d, n, ntotal,
    ...
    // -------- 2-D per-query path (fallback to scalar loop) ------------
    const float* xb = get_xb();

#pragma omp parallel for if (n > 1)
    for (idx_t qi = 0; qi < n; ++qi) {
        const float* q = x + qi * d;
        float* simi = distances + qi * k;
        idx_t* idxi = labels + qi * k;

        // Initialize heap with sentinels
        if (metric_type == METRIC_INNER_PRODUCT) {
            for (idx_t t = 0; t < k; ++t) { simi[t] = -std::numeric_limits<float>::infinity(); idxi[t] = -1; }
            heap_heapify<CMin<float, idx_t>>(k, simi, idxi);
        } else if (metric_type == METRIC_L2) {
            for (idx_t t = 0; t < k; ++t) { simi[t] =  std::numeric_limits<float>::infinity(); idxi[t] = -1; }
            heap_heapify<CMax<float, idx_t>>(k, simi, idxi);
        } else {
            // You can extend this to extra metrics if you want; for now, match existing behavior.
            // (Range path below throws for unsupported metrics as well.)
            continue;
        }

        // Build effective selector for this query:
        // adapt 2-D -> 1-D for the current query; AND with sel1d if present
        IDSelectorFrom2DForQ sel_q(sel2d, tls_query_base + qi);
        const IDSelector* eff = sel1d;
        IDSelectorAnd sel_and(nullptr, nullptr);
        if (sel1d) { sel_and = IDSelectorAnd(&sel_q, sel1d); eff = &sel_and; }
        else       { eff = &sel_q; }

        if (metric_type == METRIC_INNER_PRODUCT) {
            for (idx_t j = 0; j < ntotal; ++j) {
                if (eff && !eff->is_member(j)) continue;
                float ip = fvec_inner_product(q, xb + j * d, d);
                if (ip > simi[0]) {
            ...
            for (idx_t j = 0; j < ntotal; ++j) {
                if (eff && !eff->is_member(j)) continue;
                float dis = fvec_L2sqr(q, xb + j * d, d);
                if (dis < simi[0]) {
                    maxheap_replace_top(k, simi, idxi, dis, j);
                }
            }
...
void IndexFlat::range_search(
    ...
    if (!sel2d) {
        switch (metric_type) {
            case METRIC_INNER_PRODUCT:
                range_search_inner_product(x, get_xb(), d, n, ntotal, radius, result, sel1d);
                return;
            case METRIC_L2:
                range_search_L2sqr(x, get_xb(), d, n, ntotal, radius, result, sel1d);
                return;
            default:
                FAISS_THROW_MSG("metric type not supported");
        }
...
#pragma omp for
        for (idx_t qi = 0; qi < n; ++qi) {
            const float* q = x + qi * d;
            RangeQueryResult& qres = pres.new_result(qi);
            ...
            if (metric_type == METRIC_INNER_PRODUCT) {
                for (idx_t j = 0; j < ntotal; ++j) {
                    if (eff && !eff->is_member(j)) continue;
                    float ip = fvec_inner_product(q, xb + j * d, d);
                    if (ip > radius) qres.add(ip, j);
                }
            } else if (metric_type == METRIC_L2) {
                for (idx_t j = 0; j < ntotal; ++j) {
                    if (eff && !eff->is_member(j)) continue;
                    float dis = fvec_L2sqr(q, xb + j * d, d);
                    if (dis < radius) qres.add(dis, j);
                }
            } else {
                // Keep behavior consistent with fast path limitations
                FAISS_THROW_MSG("metric type not supported");
            }
        }
        pres.finalize();
