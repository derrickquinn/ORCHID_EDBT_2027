...
struct SearchParametersIVF : SearchParameters {
    ...
    /// context object to pass to InvertedLists
    void* inverted_list_context = nullptr;
    /// ID selector for filtering results
...
struct InvertedListScanner {
    ...
    // Add these two to support per-query 2D selectors.
    // Default no-ops so existing scanners don’t need to implement them.
    virtual void set_query_idx(idx_t) {}
    virtual void set_selector2d(const IDSelector2D*) {}
    // add end
